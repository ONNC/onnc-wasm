...
#define ONNC_RUNTIME_conv_2d(T) \
static void ONNC_RUNTIME_conv_2d_ ## T (void * restrict onnc_runtime_context, \
                                int32_t N, int32_t C, int32_t iH, int32_t iW, \
                                const T X[restrict N][C][iH][iW], \
                                int32_t M, int32_t kC, int32_t kH, int32_t kW, \
                                const T W[restrict M][kC][kH][kW], \
                                const T B[restrict M], \
                                int32_t oN, int32_t oC, int32_t oH, int32_t oW, \
                                T Y[restrict oN][oC][oH][oW], \
                                int32_t auto_pad, \
          ...
          for (int32_t w_channel = 0; w_channel < kC; ++w_channel) { \
            ...
            for (int32_t i = (base_h < 0 ? (-base_h) / dilations[0] : 0); i < kH; ++i) { \
              ...
              for (int32_t j =  (base_w < 0 ? (-base_w) / dilations[1] : 0); j < kW; ++j) { \
                ...
                if (input_w >= iW) { break; } \
                T input = X[n][input_channel][input_h][input_w]; \
                T weight = W[c][w_channel][i][j]; \
                sum += input * weight; \
              } \
            } \
          } \
...
ONNC_RUNTIME_conv_2d(float)
ONNC_RUNTIME_conv_2d(int8_t)
...
#ifndef NDEBUG
      host_QITC_time_stop("conv", "operator conv");
#endif // NDEBUG
      return;
    }
    ...
    do { // while o_dim
        ...
        do { // while w_dim
            ...
            for (int32_t channel = 0; channel < C; ++channel) {
                ...
                float weight = get_value_or_zero_float(ndim, input_W_dims, input_W, w_dim);
                sum += input * weight;
            }
            w_dim[1] = 0; // reset C
...
void ONNC_RUNTIME_conv_int8(
  ...
  ,int32_t number_of_strides
    ...
    if (ndim == 4) {
      typedef const int8_t (*input_tensor_type )[input_X_dims[1] ][input_X_dims[2] ][input_X_dims[3] ];
      typedef const int8_t (*weight_tensor_type)[input_W_dims[1] ][input_W_dims[2] ][input_W_dims[3] ];
      typedef       int8_t (*output_tensor_type)[input_W_dims[0] ][output_Y_dims[2]][output_Y_dims[3]];

      ONNC_RUNTIME_conv_2d_int8_t(onnc_runtime_context,
                                 input_X_dims[0], input_X_dims[1],
                                 input_X_dims[2], input_X_dims[3],
                                 (input_tensor_type)input_X,
                                 input_W_dims[0], input_W_dims[1],
    ...
    do { // while o_dim
        ...
        do { // while w_dim
            ...
            for (int32_t channel = 0; channel < C; ++channel) {
                ...
                int8_t weight = get_value_or_zero_int8_t(ndim, input_W_dims, input_W, w_dim);
                sum += input * weight;
            }
            w_dim[1] = 0; // reset C
        } while (next_dim(ndim, w_dim, input_W_dims));
